...
class EchoBot(sleekxmpp.ClientXMPP):
    ...
    if msg['type'] in ('chat', 'normal'):
     if 'hi' in incoming_message:
      print 'hi is present'
      msg.reply("Hello, Your Home assistant reporting from G4 Park View Apt").send()
     elif 'help' in incoming_message:
      print 'help is present'
      to_revert = helpFunction()
      msg.reply(to_revert).send()
     else:
      print 'no match found'
      msg.reply("u sent %s" %msg['body']).send()
...
if __name__ == '__main__':
    ...
    # not matter.
    xmpp = EchoBot(opts.jid, opts.password, opts.to, opts.message)
    ...
    if xmpp.connect(('xmpp.l.google.com', 5222)):
